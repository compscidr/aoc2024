...
int main(int argc, char **argv) {
  ...
  for (int l = 0; l < lines.size(); l++) {
    for (int r = 0; r < lines[l].size() ; r++) {
      if (lines[l][r] == 'X') {
        //std::cout << "Found X at (" << l << ", " << r << ")" << std::endl;
        if (findXmas(lines, 'M', l, r, UP)) {
          count++;
        }
        if (findXmas(lines, 'M', l, r, DOWN)) {
          count++;
        }
        if (findXmas(lines, 'M', l, r, LEFT)) {
          count++;
        }
        if (findXmas(lines, 'M', l, r, RIGHT)) {
          count++;
        }
        if (findXmas(lines, 'M', l, r, UP_LEFT)) {
          count++;
        }
        if (findXmas(lines, 'M', l, r, UP_RIGHT)) {
          count++;
        }
        if (findXmas(lines, 'M', l, r, DOWN_LEFT)) {
          count++;
        }
        if (findXmas(lines, 'M', l, r, DOWN_RIGHT)) {
          count++;
        }
      }
    }
